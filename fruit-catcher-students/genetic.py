...
def genetic_algorithm(individual_size, population_size, fitness_function, target_fitness,
    ...
    for generation in range(generations):
        ...
        # if generation >= STAGNATION_WINDOW:
        #     recent_best = best_fitness_history[-STAGNATION_WINDOW:]
        #     improvement = (recent_best[-1] - recent_best[0]) / (recent_best[0] + 1e-6)
            
        #     if improvement < MIN_IMPROVEMENT and (generation - last_improvement_gen) >= STAGNATION_WINDOW:
        #         stagnation_count += 1
